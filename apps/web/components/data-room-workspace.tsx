...
type DialogState =
  ...
  | { type: 'versions'; file: DataRoomFile; versions: FileVersion[] }
...
export function DataRoomWorkspace({ session, onSignedOut }: { session: Session; onSignedOut: () => void }) {
  ...
  const [searchResults, setSearchResults] = useState<FileSearchResults | null>(null);
  ...
  useEffect(() => {
    if (!active?.owner || !searchQuery.trim()) {
    ...
    const timeoutId = window.setTimeout(() => {
      void (async () => {
        setSearching(true);
        try {
          const query = new URLSearchParams({ query: searchQuery.trim(), limit: '50' });
          const result = await apiFetch<FileSearchResults>(
            `/data-rooms/${active.room.id}/files/search?${query}`,
            accessToken,
          );
          if (!cancelled) setSearchResults(result);
        } catch (caught) {
          if (!cancelled) setError(caught instanceof Error ? caught.message : 'Could not search files.');
        } finally {
  ...
  }, [accessToken, active?.owner, active?.room.id, searchQuery]);
  ...
  async function openVersions(file: DataRoomFile) {
    try {
      const versions = await apiFetch<FileVersion[]>(`/files/${file.id}/versions`, accessToken);
      setDialog({ type: 'versions', file, versions });
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'Could not load version history.');
    }
  }

  async function previewVersion(file: DataRoomFile, version: FileVersion) {
    try {
      const result = await apiFetch<{ signedUrl: string }>(
        `/files/${file.id}/versions/${version.id}/download`,
        accessToken,
      );
      setDialog({ type: 'preview', file: { ...file, name: `${file.name} — version ${version.versionNumber}` }, url: result.signedUrl });
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'Could not open this file version.');
    }
  }
  ...
  async function openMove(file: DataRoomFile) {
    setWorking(true);
    try {
      const options = (await collectFolders()).filter((target) => target.id !== file.folderId);
      if (!options.length) {
        setError('Create another folder before moving this file.');
        return;
      }
  ...
  return (
    <main className="min-h-screen bg-canvas">
        ...
        <section className="min-w-0 rounded-xl border border-slate-200 bg-white shadow-sm">
          {!active ? <EmptyState onCreate={() => setDialog({ type: 'room' })} /> : <>
            <div className="border-b border-slate-100 px-5 py-5 sm:px-7">
              ...
              {active.owner && <label className="relative mt-5 block max-w-xl"><span className="sr-only">Search files in this data room</span><input className="input pr-10" type="search" placeholder="Search file names across this data room" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} /><span className="pointer-events-none absolute right-3 top-3 text-sm text-slate-400">⌕</span></label>}
            </div>
            ...
            <div className="p-5 sm:p-7">
              {loading ? <p className="py-12 text-center text-sm text-muted">Loading contents…</p> : searchQuery.trim() ? <SearchResults results={searchResults} searching={searching} onClear={() => setSearchQuery('')} onPreview={previewFile} onOpenLocation={(file) => { setSearchQuery(''); void loadLocation(active.room, file.folderId, active.owner, false); }} /> : <FileTable folders={contents?.folders ?? []} files={contents?.files ?? []} owner={active.owner} onFolder={(folder) => void loadLocation(active.room, folder.id, active.owner, active.restricted)} onPreview={previewFile} onVersions={openVersions} onRenameFolder={(folder) => setDialog({ type: 'rename-folder', folder })} onRenameFile={(file) => setDialog({ type: 'rename-file', file })} onDeleteFolder={prepareFolderDelete} onDeleteFile={(file) => setDialog({ type: 'delete-file', file })} onMove={openMove} onShare={(resource) => setDialog({ type: 'share', resource })} />}
              {!searchQuery.trim() && contents && (contents.nextFolderCursor || contents.nextFileCursor) && <div className="mt-5 text-center"><button className="button-secondary" disabled={working} onClick={() => void loadMore()}>{working ? 'Loading…' : 'Load more items'}</button></div>}
            </div>
          </>}
        </section>
      </div>
      ...
      {dialog && <Dialogs dialog={dialog} accessToken={accessToken} working={working} onClose={() => setDialog(null)} onNewRoom={submitNewRoom} onNewFolder={submitFolder} onRename={rename} onDelete={deleteItem} onMove={moveFile} onPreviewVersion={previewVersion} />}
    </main>
  );
}
...
function FileTable({ folders, files, owner, onFolder, onPreview, onVersions, onRenameFolder, onRenameFile, onDeleteFolder, onDeleteFile, onMove, onShare }: { folders: Folder[]; files: DataRoomFile[]; owner: boolean; onFolder: (folder: Folder) => void; onPreview: (file: DataRoomFile) => void; onVersions: (file: DataRoomFile) => void; onRenameFolder: (folder: Folder) => void; onRenameFile: (file: DataRoomFile) => void; onDeleteFolder: (folder: Folder) => void; onDeleteFile: (file: DataRoomFile) => void; onMove: (file: DataRoomFile) => void; onShare: (resource: { id: string; name: string; type: 'FOLDER' | 'FILE' }) => void }) {
  ...
  return <div className="overflow-x-auto"><table className="w-full min-w-[620px] text-left text-sm"><thead className="border-b border-slate-100 text-xs uppercase tracking-wide text-slate-400"><tr><th className="px-2 py-3 font-semibold">Name</th><th className="px-2 py-3 font-semibold">Modified</th><th className="px-2 py-3 font-semibold">Size</th><th className="px-2 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody className="divide-y divide-slate-100">{folders.map((folder) => <tr key={folder.id} className="hover:bg-slate-50"><td className="px-2 py-3"><button className="flex max-w-[300px] items-center gap-2 font-medium text-ink hover:text-brand" onClick={() => onFolder(folder)}><span className="text-lg">▰</span><span className="truncate">{folder.name}</span></button></td><td className="px-2 py-3 text-muted">{formatDate(folder.updatedAt)}</td><td className="px-2 py-3 text-muted"><span>{formatBytes(folder.sizeBytes)}</span>{folder.fileCount > 0 && <span className="ml-1 text-xs text-slate-400">· {folder.fileCount} {folder.fileCount === 1 ? 'file' : 'files'}</span>}</td><td className="px-2 py-3 text-right">{owner && <RowActions actions={[['Share', () => onShare({ id: folder.id, name: folder.name, type: 'FOLDER' })], ['Rename', () => onRenameFolder(folder)], ['Delete', () => onDeleteFolder(folder), true]]} />}</td></tr>)}{files.map((file) => <tr key={file.id} className="hover:bg-slate-50"><td className="px-2 py-3"><button className="flex max-w-[300px] items-center gap-2 font-medium text-ink hover:text-brand" onClick={() => onPreview(file)}><span className="text-lg">▤</span><span className="truncate">{file.name}</span>{file.versionCount > 1 && <span className="rounded-full bg-indigo-50 px-1.5 py-0.5 text-xs font-semibold text-brand">v{file.versionCount}</span>}</button></td><td className="px-2 py-3 text-muted">{formatDate(file.updatedAt)}</td><td className="px-2 py-3 text-muted">{formatBytes(file.sizeBytes)}</td><td className="px-2 py-3 text-right">{owner ? <RowActions actions={[['Preview', () => onPreview(file)], ['Version history', () => onVersions(file)], ['Share', () => onShare({ id: file.id, name: file.name, type: 'FILE' })], ['Rename', () => onRenameFile(file)], ['Move', () => onMove(file)], ['Delete', () => onDeleteFile(file), true]]} /> : <button className="text-sm font-semibold text-brand hover:underline" onClick={() => onPreview(file)}>Preview</button>}</td></tr>)}</tbody></table></div>;
}

function SearchResults({ results, searching, onClear, onPreview, onOpenLocation }: { results: FileSearchResults | null; searching: boolean; onClear: () => void; onPreview: (file: DataRoomFile) => void; onOpenLocation: (file: DataRoomFile) => void }) {
  return <section aria-live="polite"><div className="mb-4 flex items-center justify-between gap-3"><div><h2 className="font-semibold text-ink">Search results</h2><p className="mt-1 text-sm text-muted">Matches from every folder in this data room.</p></div><button className="text-sm font-semibold text-brand hover:underline" onClick={onClear}>Clear search</button></div>{searching ? <p className="py-10 text-center text-sm text-muted">Searching files…</p> : !results?.files.length ? <p className="py-10 text-center text-sm text-muted">No files match this name.</p> : <div className="divide-y divide-slate-100 rounded-lg border border-slate-100">{results.files.map((file) => <div key={file.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3"><div className="min-w-0"><p className="truncate font-medium text-ink">▤ {file.name}{file.versionCount > 1 && <span className="ml-2 rounded-full bg-indigo-50 px-1.5 py-0.5 text-xs font-semibold text-brand">v{file.versionCount}</span>}</p><p className="mt-1 text-xs text-muted">{file.folder?.name ?? 'Data room root'} · {formatBytes(file.sizeBytes)}</p></div><div className="flex gap-3"><button className="text-sm font-semibold text-brand hover:underline" onClick={() => onOpenLocation(file)}>Show location</button><button className="text-sm font-semibold text-brand hover:underline" onClick={() => onPreview(file)}>Preview</button></div></div>)}</div>}</section>;
}
...
function RowActions({ actions }: { actions: Array<[string, () => void, boolean?]> }) {
  ...
  function toggleMenu() {
    ...
    if (!rect) return;
    setPosition({ top: rect.bottom + 6, left: Math.max(8, Math.min(window.innerWidth - 152, rect.right - 144)) });
  }

  return <><button ref={triggerRef} type="button" className="rounded-lg px-2 py-1 text-lg text-slate-500 hover:bg-slate-100" aria-label="Open item actions" aria-expanded={Boolean(position)} onClick={toggleMenu}>⋯</button>{position && typeof document !== 'undefined' && createPortal(<div ref={menuRef} role="menu" className="fixed z-[60] w-36 rounded-lg border border-slate-200 bg-white py-1 shadow-xl" style={position}>{actions.map(([label, onClick, danger]) => <button key={label} type="button" role="menuitem" className={`block w-full px-3 py-2 text-left text-sm hover:bg-slate-50 ${danger ? 'text-rose-600' : 'text-slate-700'}`} onClick={() => { setPosition(null); onClick(); }}>{label}</button>)}</div>, document.body)}</>;
}

function Dialogs({ dialog, accessToken, working, onClose, onNewRoom, onNewFolder, onRename, onDelete, onMove, onPreviewVersion }: { dialog: DialogState; accessToken: string; working: boolean; onClose: () => void; onNewRoom: (event: FormEvent<HTMLFormElement>) => void; onNewFolder: (event: FormEvent<HTMLFormElement>) => void; onRename: (target: Folder | DataRoomFile, isFolder: boolean, event: FormEvent<HTMLFormElement>) => void; onDelete: (path: string) => void; onMove: (file: DataRoomFile, event: FormEvent<HTMLFormElement>) => void; onPreviewVersion: (file: DataRoomFile, version: FileVersion) => void }) {
  ...
  if (dialog.type === 'versions') return <Modal title={`Version history — ${dialog.file.name}`} onClose={onClose}><p className="-mt-2 mb-4 text-sm text-muted">Uploading the same name in this folder creates a new version rather than another file.</p><ul className="divide-y divide-slate-100">{dialog.versions.map((version) => <li key={version.id} className="flex items-center justify-between gap-3 py-3"><span><strong className="block text-sm text-ink">Version {version.versionNumber}{version.versionNumber === dialog.file.versionCount && <span className="ml-2 rounded-full bg-emerald-50 px-1.5 py-0.5 text-xs text-emerald-700">Current</span>}</strong><span className="text-sm text-muted">{formatDate(version.createdAt)} · {formatBytes(version.sizeBytes)}</span></span><button type="button" className="text-sm font-semibold text-brand hover:underline" onClick={() => onPreviewVersion(dialog.file, version)}>Preview</button></li>)}</ul></Modal>;
