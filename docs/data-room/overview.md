...
- `apps/api/prisma/schema.prisma` — metadata model, including logical files and their immutable versions.
...
4. For an upload, the owner requests a signed storage upload URL, uploads to the private bucket from the browser, and calls the completion endpoint. The API checks that the expected object exists before creating metadata. If that name already exists in the same folder, the API appends an immutable version and makes it current instead of creating a suffixed duplicate.
...
- PDF files are limited to 50 MB in this MVP. Folder rows show the total byte size and file count across their whole nested subtree.
- Search matches the normalized file name across a Data Room and returns the containing folder, while version history exposes signed previews for current and older versions.
